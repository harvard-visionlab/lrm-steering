...
Deep neural networks have achieved remarkable success in object recognition tasks and serve as visual front-end encoders for larger-scale cognitive architectures.  However, standard models lack the ability to influence visual encoding based on cognitive-level goals. Inspired by biological and cognitive science evidence, we introduce cognitive steering capabilities to deep convolutional neural network models using long-range modulatory feedback connections and multiplicative (as opposed to additive) modulation. Our approach involved training several models with top-down modulatory feedback projections, where both the feedforward and the modulated pass support accurate object recognition. We observe improved ImageNet recognition accuracy and adversarial robustness in the modulated pass, suggesting these long-range connections naturally refine visual encoding.  The top-down connections also provide an interface for cognitive systems to steer the visual encoding based on an attentional set or goal. We investigate several steering approaches, including a language-based steering signal, and quantify the efficacy of the cognitive steering capacity using composite image tests. To test steerability, we constructed a composite image challenge where the goal is to classify images depicting two distinct object categories: all models show poor recognition accuracy for either category under traditional feed-forward encoding, but LRM models can report the presence of either category with substantially greater accuracy with cognitive steering, and they do so without hallucinating the target-objects that are absent from the image. Finally, we show that modulatory feedback can modulate the activity of category-selective model units, mirroring phenomena observed in human visual processing. Broadly, this work demonstrates that incorporating long-range modulatory feedback connections into feed forward computer vision models enables new behavioral capacities for goal-directed visual encoding.    
...
To provide a "steering signal", all you need is a representation of the current "steering template" and to specify which feedback source this template is replacing (in our work, we only replace the final network output with steering templates, capturing the notion that it's the final stages of the visual system that interface with the cognitive system). 
...
<div align="center" width="600px">
    <img src="images/composite.png" alt="Composite Image Challenge" width="600px">
    <p><em>Composite Image Challenge.</em> Three images from different classes are selected. Two of the images are combined to form a composite image. Effective steering should be able to amplify either of the classes that are present (497, 574), but shouldn't hallucinate classes that are absent (e.g., 0).</p>
</div>
